...
def read_book(path, dt = 1000):
    ...
    with open("output.txt", "w") as output_file:
        ...
        for date in sorted(os.listdir(path)):
            with open(f"{DIR}/{perp}/{date}") as file:
                start_time = None
                dif = np.nan
                ...
                for i, line in enumerate(file):
                    ...
                    if json_data["type"] == "snapshot":
                        ...
                        for order in snapshot["b"]:
                            bid_p = float(order[0])
                            bids[bid_p] = float(order[1])
                            max_bid = max(max_bid, bid_p)
                        for order in snapshot["a"]:
                            ask_p = float(order[0])
                            asks[ask_p] = float(order[1])
                        ...
                        for order in row["b"]:
                            bid, vol = float(order[0]), float(order[1])
                        ...
                        for order in row["a"]:
                            ask, vol = float(order[0]), float(order[1])
...
if __name__ == "__main__":
    for perp in os.listdir(DIR):
        read_book(f"{DIR}/{perp}")  
        break      
